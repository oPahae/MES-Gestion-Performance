...
export default async function handler(req, res) {
  ...
  if (!allowed) {
    res.status(403).json({ error: "Vous n'êtes pas autorisé à valider ce poste." });
    return;
  }
  ...
  try {
    ...
    await query(
      `UPDATE workflow_run_etapes re JOIN workflow_etapes e ON e.id = re.etape_id
       SET re.valide = 1, re.valide_at = NOW()
       WHERE re.run_id = ? AND e.poste_id = ?`,
      [id, posteId]
    );
